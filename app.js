...
// Pulled in ADDITION to the plot section (never instead of) -- real prose
// about characters/setting often names props and belongings a bare plot
// summary skips, giving a short synopsis more genuine material for the
// quest-object pool without inventing anything.
const WIKI_SUPPLEMENTARY_SECTIONS = [
  "characters", "setting", "personajes", "ambientacion", "escenario",
  "figuren", "schauplatz", "personnages", "lieu", "cadre"
];
...
const I18N = {
  "en-GB": {
    ...
    cloudAiTitle: "Cloud AI",
    cloudAiToggleLabel: "Allow cloud AI for better quest objects (sends the book's title, author and synopsis to a cloud service)",
    cloudAiHint: "Only used when this deployment has a backend configured. Falls back to on-device AI otherwise.",
    ...
    questSourcePoolCached: "From this book's shared object collection",
    questSourceCloudAi: "Generated by AI (cloud)",
    ...
    aiStatusIdle: "Chrome built-in AI not available on this browser.",
    ...
    rereadStarted: "Started a new reading of “{title}”",
    rereadSectionTitle: "Reread a finished book",
    rereadSelectPlaceholder: "Choose a finished book…",
    rereadButton: "Start reread",
    passLabel: "reading {n}",
    readBooksTitle: "Read books",
    readBooksEmpty: "No finished books yet.",
    readBooksTimesRead: "Read {n} time(s)",
    readBooksInProgress: "in progress",
    readBooksFinishedOn: "finished {date}",
    readBooksViewStats: "View stats",
...
const state = {
  ...
  sessions: [],
  quotes: [],
  // Per-work (title+author) pool of validated quest objects, shared across
  // every reading pass of that book. See computeWorkId / mergeIntoQuestPool.
  questPools: {},
  timer: {
    running: false,
    mode: "sprint",
...
function getQuestAppKey(){
  if(typeof window === "undefined") return "";
  return (window.BOOKQUEST_CONFIG && window.BOOKQUEST_CONFIG.questAppKey) || "";
}

function cloudAiEnabled(){
  return backendEnabled() && state.settings.cloudAiEnabled !== false;
}
...
function load(){
  ...
  state.settings = Object.assign({ lang:"en-GB", aiEnabled:true, aiEnabledMigrated:false, usePlotSummaries:true, cloudAiEnabled:true }, state.settings || {});
...
// The stable identity of a "work" (same story, any edition/printing) used to
// link multiple readings of the same book and share their quest-object pool.
// Frozen at creation, not recomputed on every load, so fixing a typo in the
// title later doesn't silently unlink a book from its own reading history.
function computeWorkId(title, author){
  return String(hashString(`${normalizeMatchStr(title)}|${normalizeMatchStr(author)}`));
}
...
function normalizeBooks(){
  for(const id of Object.keys(state.books || {})){
    ...
    if(!b.workId) b.workId = computeWorkId(b.title, b.author);
    if(b.totalPages && (b.currentPage || 0) >= b.totalPages && !b.finishedAt){
      b.finishedAt = new Date().toISOString();
    }
  }
  // Books saved before rereads existed (or a pre-existing accidental
  // duplicate of the same title) never got a passNumber -- assign one now,
  // ordered by when each was created, without renumbering anything already set.
  const byWork = {};
  for(const id of Object.keys(state.books || {})){
    const b = state.books[id];
    (byWork[b.workId] || (byWork[b.workId] = [])).push(b);
  }
  for(const workId of Object.keys(byWork)){
    const group = byWork[workId];
    const missing = group.filter(b => !Number.isFinite(b.passNumber));
    if(!missing.length) continue;
    const maxExisting = group.reduce((m, b) => Number.isFinite(b.passNumber) ? Math.max(m, b.passNumber) : m, 0);
    missing.sort((a, b) => new Date(a.createdAt || 0) - new Date(b.createdAt || 0));
    missing.forEach((b, idx) => { b.passNumber = maxExisting + idx + 1; });
  }
  if(!state.questPools || typeof state.questPools !== "object") state.questPools = {};
}
...
async function fetchWikipediaPlot(title, author, ...langCandidates){
  ...
  for(const code of langs){
    try{
      ...
      if(!plot) continue;
      const extras = sections.filter(sec =>
        sec !== plot && WIKI_SUPPLEMENTARY_SECTIONS.includes(normalizeMatchStr(sec.line))
      );
      const wanted = [plot, ...extras];
      const parts = [];
      for(const sec of wanted){
        try{
          const txtData = await wikiJson(code, { action: "parse", page, prop: "text", section: sec.index });
          const html = (txtData && txtData.parse && txtData.parse.text) ? txtData.parse.text["*"] : "";
          const cleaned = cleanWikiText(html);
          if(cleaned) parts.push(cleaned);
        }catch(_){}
      }
      const text = truncatePlot(parts.join(" "));
      if(text.length >= 120){
        return { text, lang: code, article: page, section: wanted.map(s => s.line).join(", ") };
      }
    }catch(_){}
  }
  return null;
...
// Sanity ceiling on how large a single work's quest-object pool can grow --
// a safeguard against runaway storage/abuse, not a target to reach. Most
// books settle for good at whatever the source text actually supports.
const QUEST_POOL_MAX = 50;

function getQuestPool(workId){
  if(!workId) return [];
  const entry = state.questPools && state.questPools[workId];
  return entry && Array.isArray(entry.objects) ? entry.objects : [];
}

// Folds newly-validated objects into a work's shared pool (dedup, capped),
// regardless of which engine produced them -- cloud, Chrome AI, or the
// heuristic extractor all feed the same pool so a reread benefits from
// whichever source got lucky, not just the one that ran this time.
function mergeIntoQuestPool(workId, newObjects){
  if(!workId) return [];
  const existing = getQuestPool(workId);
  ...
  for(const obj of (newObjects || [])){
    ...
    seen.add(key);
    merged.push(obj);
    if(merged.length >= QUEST_POOL_MAX) break;
  }
  state.questPools[workId] = { objects: merged, updatedAt: new Date().toISOString() };
  return merged;
}

// Draws this reading pass's subset from the shared pool. Same seeded
// Fisher-Yates pattern as pickQuestFallbackObjects, just shuffling the real
// pool instead of the generic fallback list -- each pass has its own
// book.id, so questSeedForBook already gives every pass a distinct draw.
function pickFromQuestPool(book, count){
  const pool = getQuestPool(book && book.workId).slice();
...
function sanitizeQuestObjects(raw, count){
  ...
  // Generation now asks for an open-ended extraction ("find every object
  // this text supports, up to `count`"), not an exact count -- a book that
  // only genuinely offers 4 objects out of 50 asked for is the correct,
  // expected result, not a sign the model failed. So the floor here is
  // absolute (at least one real object survived), not relative to `count`.
  ...
  return unique.length > target ? unique.slice(0, target) : unique;
}
...
function questPromptForSynopsis(count, lang){
  const n = count || QUEST_POOL_MAX;
  const langName = langCodeToName(lang);
  // Open-ended extraction, not a fixed quota: a short synopsis that only
  // supports 4 real objects should return 4, not be padded to n. Kept
  // deliberately short otherwise -- the on-device models this runs against
  // are as small as Gemini Nano, and a longer, multi-clause instruction made
  ...
  return [
    "You are given a book plot summary.",
    `Find every concrete physical object clearly supported by the text, up to ${n}, one per key scene, earliest first. Fewer is fine if the text doesn't support more.`,
    ...
    `Reply with ONLY a JSON array of short lowercase names in ${langName}. No extra text.`
  ].join(" ");
}
...
// A capitalized word that IS a common physical-object noun -- "Stone" in
// "the Philosopher's Stone", "Mirror" in "the Mirror of Erised" -- is a named
// ITEM, not a person or organization, even though it looks identical to
// "Genialissimus" or "Kartsev" by capitalization alone. Recovering these is
// what actually gets book-specific, named objects (not just "wall"/"horse")
// out of the heuristic extractor. Deliberately a closed, universal-ish class
// (things you can hold, wear, wield or carry) rather than an attempt at full
// vocabulary coverage -- EN + ES, like the other curated lists in this file.
const QUEST_PHYSICAL_NOUN_ROOTS = new Set([
  "stone","wand","cloak","sword","ring","map","mirror","key","book","potion",
  "cauldron","broom","crown","staff","bottle","letter","coin","box","chest",
  "cup","cake","hat","cape","dagger","knife","shield","helmet","armor","boot",
  "boots","glove","gloves","mask","veil","lamp","candle","clock","watch",
  "scroll","amulet","medallion","pendant","necklace","bracelet","locket",
  "flute","harp","drum","bell","jar","vase","gem","chain","rope","net",
  "wheel","ship","boat","train","car","horse","wall","door","gate","bridge",
  "castle","tower","sword","bow","arrow","spear","axe","hammer","ticket",
  "badge","flag","banner","seal","stamp","uniform","medal","diary","journal",
  "notebook","pen","pencil","photograph","picture","painting","statue",
  "piedra","varita","capa","espada","anillo","mapa","espejo","llave","libro",
  "pocion","caldero","escoba","corona","baston","botella","carta","moneda",
  "caja","cofre","copa","pastel","sombrero","daga","cuchillo","escudo","casco",
  "armadura","bota","botas","guante","guantes","mascara","velo","lampara",
  "vela","reloj","pergamino","amuleto","medallon","colgante","collar",
  "pulsera","relicario","flauta","arpa","tambor","campana","jarra","florero",
  "gema","cadena","cuerda","red","rueda","barco","tren","caballo","muro",
  "puerta","puente","castillo","torre","arco","flecha","lanza","hacha",
  "martillo","boleto","insignia","bandera","sello","uniforme","medalla",
  "diario","cuaderno","pluma","lapiz","fotografia","cuadro","estatua"
]);
...
function properNounWordsIn(text){
  ...
  for(const sentence of sentences){
    ...
    words.forEach((w, idx) => {
      ...
      const looksLikeNamedItem = QUEST_PHYSICAL_NOUN_ROOTS.has(w.toLowerCase());
      if((isAllCaps || isCapitalized) && !looksLikeNamedItem) proper.add(w.toLowerCase());
    });
  }
  return proper;
}
...
function getAiStatusText(){
  ...
  // Chrome Built-in AI is the only on-device engine left -- WebLLM and
  // Transformers.js were removed once the shared cloud tier (tier 0) made
  // them redundant: strictly lower quality, and the source of most of the
  // download/stall bugs this app had before cloud AI existed.
...
function updateAiUI(){
  ...
  const cloudToggle = $("cloudAiToggle");
  if(cloudToggle){
    cloudToggle.checked = state.settings.cloudAiEnabled !== false;
    cloudToggle.disabled = !backendEnabled();
  }
  if(!status) return;
...
async function requestCloudQuestObjects(book, count, lang, sourceText, sameLanguage){
  try{
    const headers = { "Content-Type": "application/json" };
    const appKey = getQuestAppKey();
    if(appKey) headers["X-App-Key"] = appKey;
    const res = await fetch(`${getBackendUrl()}/quest/objects`, {
      method: "POST",
      headers,
      body: JSON.stringify({
        title: book.title || "",
    ...
    const data = await res.json();
    // The Worker returns the whole shared pool, not just this request's
    // slice -- mergeIntoQuestPool/pickFromQuestPool handle the local draw.
    return Array.isArray(data && data.pool) ? data.pool : null;
  }catch(_){
    return null;
  }
}
...
async function generateQuestObjectsForBook(book, options){
  ...
  if(!state.books || !state.books[book.id]) return;
  ensureQuestDefaults(book);
  if(!book.workId) book.workId = computeWorkId(book.title, book.author);
  ...
  // Persists newObjects into the work's shared pool (skipped for the
  // untargeted emergency list -- see pickQuestFallbackObjects's caller --
  // since that one isn't actually about THIS book and would pollute every
  // other reader's pool with generic placeholders), then draws this pass's
  // slice from whatever the pool now holds.
  function commitObjects(newObjects, method, opts){
    if(!state.books || !state.books[book.id]) return false;
    const feedsPool = !(opts && opts.skipPoolMerge);
    if(feedsPool && newObjects && newObjects.length){
      mergeIntoQuestPool(book.workId, newObjects);
    }
    const drawn = feedsPool ? pickFromQuestPool(book, count) : newObjects;
    book.quest.objects = drawn.length ? drawn : newObjects;
  ...
  try{
    // If this work's shared pool already covers what this pass needs to
    // show, skip straight to drawing a subset -- no AI call, no cost, and a
    // reread of a book others already generated feels instant.
    const existingPool = getQuestPool(book.workId);
    if(existingPool.length >= count){
      setQuestStatus(book.id, `pool already has ${existingPool.length} objects → drawing a fresh subset`);
      commitObjects(pickFromQuestPool(book, count), "pool-cached", { skipPoolMerge: true });
      return;
    }
    ...
    if(!synopsis){
      if(allowFallback){
        if(existingPool.length){
          setQuestStatus(book.id, "no synopsis → drawing from existing pool");
          commitObjects(pickFromQuestPool(book, count), "pool-cached", { skipPoolMerge: true });
        }else{
          setQuestStatus(book.id, "no synopsis → generic pool (AI never ran)");
          const fallback = pickQuestFallbackObjects(book, count);
          if(fallback.length) commitObjects(fallback, "pool", { skipPoolMerge: true });
        }
      }
      return;
    }
    ...
    // Ask each engine to extract everything the text genuinely supports, up
    // to the pool's sanity ceiling -- not the small per-pass `count`, which
    // only governs how many of the pool's objects THIS reading unlocks.
    const askCount = QUEST_POOL_MAX;

    // 0. Cloud AI (shared Cloudflare Worker, cached per-book across every
    //    user) — tried first when online and a backend is configured for
    //    this deployment. Falls through to the on-device tiers below on ANY
    //    failure: network error, daily budget exceeded, invalid response, or
    //    backendUrl left empty (self-hosted copies of this app with no
    //    Worker configured skip this tier entirely, for free).
    if(wantsAI && cloudAiEnabled() && online){
      setQuestStatus(book.id, "requesting cloud AI objects");
      let objects = null;
      const raw = await requestCloudQuestObjects(book, askCount, lang, synopsis, sameLang);
      recordAiRaw(book.id, "cloud-ai", raw, null);
      objects = sanitizeQuestObjects(raw, askCount);
      if(_aiRawLog[book.id]) _aiRawLog[book.id].afterFormat = objects;
      if(objects && objects.length){
        // Defense in depth: the Worker already grounds its own response, but
        // re-checking here costs nothing and catches any divergence between
        // the two implementations (they can't share code across the
        // browser/Worker boundary without a build step).
        ...
        if(!grounded.length){
          note("cloud AI output not grounded in source text → discarded");
        }
        objects = grounded;
      }
      if(objects && objects.length){
        setQuestStatus(book.id, "cloud AI success");
        commitObjects(objects, "cloud-ai");
        return;
      }
      note("cloud AI unavailable → trying on-device");
    }
    ...
    if(wantsAI && windowAiSupported()){
      ...
      let lastRaw = null;
      try{
        const raw = await requestWindowAiQuestObjects(book, askCount, lang, synopsis);
        lastRaw = raw;
        objects = sanitizeQuestObjects(raw, askCount);
        if(!objects){
          setQuestStatus(book.id, "Chrome AI retry");
          const retryRaw = await requestWindowAiQuestObjects(book, askCount, lang, synopsis);
          lastRaw = retryRaw;
          objects = sanitizeQuestObjects(retryRaw, askCount);
        }
      }catch(_){}
    ...
    // 2. Heuristic / pool fallback
    if(!allowFallback){
      setQuestStatus(book.id, "AI failed → keeping existing fallback");
      return;
    }
    const heuristic = heuristicQuestObjectsFromSynopsis(synopsis, book.quest && book.quest.seed, askCount);
    if(heuristic.length){
      // Grounded by construction (extracted straight from the real
      // synopsis), so -- like the AI tiers -- it's trustworthy enough to
      // feed the shared pool, just less curated.
      const reason = !wantsAI ? "ai disabled" : !online ? "offline"
        : attempts.length ? attempts[attempts.length - 1] : "ai unavailable";
      setQuestStatus(book.id, `${reason} → heuristic`);
      commitObjects(heuristic, "heuristic");
      return;
    }
    if(existingPool.length){
      setQuestStatus(book.id, "heuristic found nothing → drawing from existing pool");
      commitObjects(pickFromQuestPool(book, count), "pool-cached", { skipPoolMerge: true });
      return;
    }
    const fallback = pickQuestFallbackObjects(book, count);
    if(fallback.length){
      const reason = !wantsAI ? "ai disabled" : !online ? "offline"
        : attempts.length ? attempts[attempts.length - 1] : "ai unavailable";
      setQuestStatus(book.id, `${reason} → generic pool`);
      commitObjects(fallback, "pool", { skipPoolMerge: true });
    }
  }finally{
...
// Whether a work (title+author) has more than one linked reading pass --
// gates showing the "(reading N)" suffix so a book that's only ever been
// read once doesn't get a confusing label.
function workHasMultiplePasses(workId){
  let count = 0;
...
function bookSelectLabel(book){
  const base = book.title || t("untitled");
  if(Number.isFinite(book.passNumber) && book.passNumber > 0 && workHasMultiplePasses(book.workId)){
    return `${base} (${t("passLabel", { n: String(book.passNumber) })})`;
  }
  return base;
}
...
function refreshBookSelect(){
  ...
  sel.innerHTML = ids.map(id=>{
    const b = state.books[id];
    return `<option value="${id}">${bookSelectLabel(b)}</option>`;
  }).join("");
...
function renderSessionBookSelect(){
  ...
  select.innerHTML = books.map(b => `<option value="${b.id}">${bookSelectLabel(b)}</option>`).join("");
...
function questMethodInfo(method){
  switch(method){
    case "pool-cached":    return { key: "questSourcePoolCached",   good: true };
    case "cloud-ai":       return { key: "questSourceCloudAi",      good: true };
...
function renderQuestDebug(book){
  ...
  const lines = [
    ...
    `Cloud AI: ${backendEnabled() ? (cloudAiEnabled() ? "enabled" : "disabled by toggle") : "no backend configured"}`,
...
// Groups state.books by workId -- one vault card per book (title+author),
// not per reading pass, since every pass of the same work draws from and
// unlocks against the same shared object pool.
function groupBooksByWork(){
  const byWork = {};
  for(const b of Object.values(state.books || {})){
    if(!b) continue;
    (byWork[b.workId] || (byWork[b.workId] = [])).push(b);
  }
  return Object.values(byWork).map(passes =>
    passes.slice().sort((a, b) => (a.passNumber || 0) - (b.passNumber || 0))
  );
}
...
function renderVault(){
  ...
  const openWorkIds = new Set(Array.from(container.querySelectorAll(".vault-item[open]")).map(el => el.dataset.workId));
  if(!state.ui.vaultReveal) state.ui.vaultReveal = {};
  const worksHtml = groupBooksByWork().map(passes => {
    const latest = passes[passes.length - 1];
    const workId = latest.workId;
    const reveal = Boolean(state.ui.vaultReveal[workId]);
    const pool = getQuestPool(workId);
    // Union of whatever each individual pass has unlocked -- different
    // passes can have drawn (and thus unlocked) different subsets of the
    // pool, so a reread can reveal something the first reading never did.
    const unlockedLabels = new Set();
    let maxItemCount = pool.length;
    for(const b of passes){
      ...
      maxItemCount = Math.max(maxItemCount, objects.length, thresholds.length || 0);
      let unlocked = 0;
      for(const tVal of thresholds){ if(progress >= tVal) unlocked += 1; }
      for(let i = 0; i < Math.min(unlocked, objects.length); i++) unlockedLabels.add(objects[i]);
    }
    const itemCount = Math.max(maxItemCount, QUEST_THRESHOLDS.length);
    const unlockedCount = unlockedLabels.size;
    const unlockedList = Array.from(unlockedLabels);
    const openAttr = openWorkIds.has(workId) ? "open" : "";
    const authorLine = latest.author ? `<div class="vault-author">${latest.author}</div>` : "";
    const objectsHtml = Array.from({ length: itemCount }, (_, idx) => {
      const isUnlocked = idx < unlockedList.length;
      const label = (isUnlocked || reveal) ? (unlockedList[idx] || pool[idx] || "???") : "???";
      const icon = isUnlocked ? "✅" : "⬜";
    ...
    return `
      <details class="vault-item" data-work-id="${workId}" data-book-id="${latest.id}" ${openAttr}>
        <summary class="vault-header">
          <div>
            <div class="vault-title">${latest.title || t("untitled")}</div>
            ${authorLine}
          </div>
          <div class="vault-chip">${t("vaultObjects")}: ${Math.min(unlockedCount, itemCount)}/${itemCount}</div>
        </summary>
...
function renderRereadSelect(){
  const select = $("rereadSelect");
  if(!select) return;
  const previous = select.value;
  const works = finishedWorks();
  select.innerHTML = `<option value="" data-i18n="rereadSelectPlaceholder">${t("rereadSelectPlaceholder")}</option>` +
    works.map(b => `<option value="${b.id}">${b.title || t("untitled")}${b.author ? " — " + b.author : ""}</option>`).join("");
  if(works.some(b => b.id === previous)) select.value = previous;
}

function renderReadBooksSelect(){
  const select = $("readBooksSelect");
  if(!select) return;
  const previous = select.value;
  const works = finishedWorks();
  if(!works.length){
    select.innerHTML = "";
    const info = $("readBooksInfo");
    if(info) info.textContent = t("readBooksEmpty");
    return;
  }
  select.innerHTML = works.map(b => `<option value="${b.workId}">${b.title || t("untitled")}${b.author ? " — " + b.author : ""}</option>`).join("");
  if(works.some(b => b.workId === previous)) select.value = previous;
  renderReadBooksInfo();
}

function renderReadBooksInfo(){
  const info = $("readBooksInfo");
  const select = $("readBooksSelect");
  ...
  const passes = Object.values(state.books || {})
    .filter(b => b.workId === workId)
    .sort((a, b) => (a.passNumber || 0) - (b.passNumber || 0));
  const timesRead = passes.filter(b => b.finishedAt).length;
  const rows = passes.map(b => {
    const status = b.finishedAt
      ? t("readBooksFinishedOn", { date: new Date(b.finishedAt).toLocaleDateString() })
      : t("readBooksInProgress");
    const label = t("passLabel", { n: String(b.passNumber || 1) });
    return `<div class="row" data-book-id="${b.id}">
      <div class="grow">${label} — ${status}</div>
      <button class="btn" type="button" data-view-stats="${b.id}">${t("readBooksViewStats")}</button>
    </div>`;
  }).join("");
  info.innerHTML = `<div class="small">${t("readBooksTimesRead", { n: String(timesRead) })}</div>${rows}`;
}

// ---------- CRUD Books ----------
// Next reading-pass number for a work (title+author identity) -- 1 for a
// brand-new work, one past whatever pass already exists otherwise.
function nextPassNumber(workId){
  let max = 0;
  for(const b of Object.values(state.books || {})){
    if(b.workId === workId && Number.isFinite(b.passNumber)) max = Math.max(max, b.passNumber);
  }
  return max + 1;
}
...
function addBook(){
  ...
  const id = uid();
  const workId = computeWorkId(title, author);
  state.books[id] = {
    id,
    workId,
    passNumber: nextPassNumber(workId),
    title,
    author,
    publisher,
    edition,
...
// Every finished book (any pass with finishedAt set), one entry per work --
// used to populate the "reread" picker without listing the same title once
// per prior pass.
function finishedWorks(){
  const byWork = {};
  for(const b of Object.values(state.books || {})){
    if(!b || !b.finishedAt) continue;
    const existing = byWork[b.workId];
    if(!existing || (b.passNumber || 0) > (existing.passNumber || 0)){
      byWork[b.workId] = b;
    }
  }
  return Object.values(byWork).sort((a, b) => (a.title || "").localeCompare(b.title || ""));
}

// Starts a new, independent reading pass of a book the user already
// finished: its own progress/stats from page 0, same workId (so it keeps
// sharing/growing the quest-object pool with every other pass of this work),
// nothing about the original pass touched or deleted.
function startReread(sourceBookId){
  const source = state.books && state.books[sourceBookId];
  if(!source) return;
  const id = uid();
  state.books[id] = {
    id,
    workId: source.workId || computeWorkId(source.title, source.author),
    passNumber: nextPassNumber(source.workId),
    title: source.title,
    ...
    synopsisManual: Boolean(source.synopsisManual),
    language: source.language || "",
    plotSummary: source.plotSummary || "",
    plotSource: source.plotSource ? Object.assign({}, source.plotSource) : { lang:"", article:"", fetchedAt:"" },
    googleBooks: source.googleBooks ? Object.assign({}, source.googleBooks) : { id: "", fetchedAt: "" },
    metaSource: source.metaSource ? Object.assign({}, source.metaSource) : null,
    quest: { objects: [], generatedAt: "", method: "", thresholds: QUEST_THRESHOLDS.slice(), seed: 0 }
  };
  state.activeBookId = id;
  save();
  renderAll();
  showToast(t("rereadStarted", { title: source.title || t("untitled") }));
  // Already has title/author/synopsis -- straight to quest generation, no
  // provider lookup needed. Almost always resolves from the shared pool
  // instantly, since this exact work was already read once.
...
function bind(){
  ...
  const startRereadBtn = $("startReread");
  if(startRereadBtn){
    startRereadBtn.addEventListener("click", () => {
      const sourceId = $("rereadSelect").value;
      if(!sourceId) return;
      startReread(sourceId);
    });
  }
  const readBooksSelect = $("readBooksSelect");
  if(readBooksSelect){
    readBooksSelect.addEventListener("change", renderReadBooksInfo);
  }
  const readBooksInfo = $("readBooksInfo");
  if(readBooksInfo){
    readBooksInfo.addEventListener("click", (ev) => {
      const btn = ev.target.closest("[data-view-stats]");
      if(!btn) return;
      const id = btn.getAttribute("data-view-stats");
      if(!state.books[id]) return;
      state.activeBookId = id;
      state.ui.quotesBookId = id;
      save();
      renderAll();
    });
  }
  ...
  const cloudAiToggle = $("cloudAiToggle");
  if(cloudAiToggle){
    cloudAiToggle.addEventListener("change", () => {
      state.settings.cloudAiEnabled = cloudAiToggle.checked;
      save();
      updateAiUI();
    });
  }
  ...
  if(typeof window !== "undefined"){
    window.addEventListener("online", updateAiUI);
  ...
  if(vault){
    vault.addEventListener("click", (e) => {
      ...
      if(actionBtn && vault.contains(actionBtn)){
        ...
        if(action === "reveal"){
          if(!state.ui.vaultReveal) state.ui.vaultReveal = {};
          state.ui.vaultReveal[workId] = !state.ui.vaultReveal[workId];
          save();
          renderAll();
          return;
        }
        if(action === "delete"){
          // The pool and every pass's drawn subset are shared across this
          // whole work, so "delete" resets all of it, not just one reading.
          delete state.questPools[workId];
          for(const b of Object.values(state.books || {})){
            if(b.workId !== workId) continue;
            ensureQuestDefaults(b);
            b.quest.objects = [];
            b.quest.generatedAt = "";
            b.quest.method = "";
...
if(typeof window !== "undefined"){
  window.BOOKQUEST_TEST_API = {
    ...
    updateTimerDisplay,
    formatMMSS,
    generateQuestObjectsForBook,
    requestCloudQuestObjects,
    sanitizeQuestObjects,
    groundQuestObjects,
    backendEnabled,
    cloudAiEnabled,
    ensureQuestDefaults,
