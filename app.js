...
const BUILD_VERSION = "Build v1.6.0-quote-ocr";

// Google Drive OAuth Client ID
// Preferred: window.BOOKQUEST_CONFIG.googleClientId (set in config.js)
// Fallback: baked-in client ID (works only if it is still valid in your Google Cloud project)
const GOOGLE_CLIENT_ID_FALLBACK = "195858719729-36npag3q1fclmj2pnqckk4dgcblqu1f9.apps.googleusercontent.com";
...
const TRANSLATIONS = {
  en: {
    ...
    // Quote photo OCR UX
    btn_add_photo: "Add photo (optional)",
    btn_choose_file: "Choose from device",
    btn_take_photo: "Take a photo",
    hint_photo_optional: "Add a photo if you want to crop & extract text into the quote box."
  },
  es: {
    ...
    // Quote photo OCR UX
    btn_add_photo: "Agregar foto (opcional)",
    btn_choose_file: "Elegir del dispositivo",
    btn_take_photo: "Tomar foto",
    hint_photo_optional: "Agrega una foto si quieres recortar y extraer texto dentro de la caja de cita."
  }
};
...
const state = {
  ...
  drive: { token:null, fileId:null, lastSyncISO:null, clientId:"" },
  quotes: [] 
};

function getGoogleClientId(){
  const fromState = (state?.drive?.clientId || "").trim();
  const fromConfig = (window.BOOKQUEST_CONFIG?.googleClientId || "").trim();
  return fromState || fromConfig || GOOGLE_CLIENT_ID_FALLBACK;
}
...
function load(){
  ...
  try{
    ...
    if(!state.drive) state.drive = { token:null, fileId:null, lastSyncISO:null, clientId:"" };
    if(state.drive.clientId == null) state.drive.clientId = "";
  }catch(_){}
}
...
function driveTokenClient(){
  ...
  return google.accounts.oauth2.initTokenClient({
    client_id: getGoogleClientId(), scope: SCOPE,
    callback: (resp) => {
...
function bind(){
  ...
  // Quotes: optional photo -> crop -> OCR into quoteText
  let cropper=null;
  const openCropperFromFile = (f)=>{
    if(!f) return;
    const r=new FileReader();
    r.onload=()=>{
      $("imageToCrop").src=r.result;
  ...
  const toggle = $("btnQuotePhotoToggle");
  const controls = $("quotePhotoControls");
  if(toggle && controls){
  ...
  const pickFileBtn = $("btnQuotePickFile");
  const takePhotoBtn = $("btnQuoteTakePhoto");
  const fileInput = $("quoteImageFile");
  const camInput = $("quoteImageCamera");
  if(pickFileBtn && fileInput) pickFileBtn.addEventListener("click",()=>fileInput.click());
  if(takePhotoBtn && camInput) takePhotoBtn.addEventListener("click",()=>camInput.click());

  if(fileInput) fileInput.addEventListener("change",(e)=>{ openCropperFromFile(e.target.files?.[0]); e.target.value=""; });
  if(camInput) camInput.addEventListener("change",(e)=>{ openCropperFromFile(e.target.files?.[0]); e.target.value=""; });

  $("btnScanText").addEventListener("click", async()=>{
    if(!cropper) return;
    $("ocrStatus").textContent=t("status_scanning");
    const dataUrl=cropper.getCroppedCanvas({maxWidth:1600,maxHeight:1600,fillColor:"#fff"}).toDataURL("image/png");
    try{
      // Try English + Spanish if available; fall back to English.
      const recognize = async (lang)=> Tesseract.recognize(dataUrl, lang, {
        logger: m => {
          if(m?.status){
      ...
      let text="";
      try{ ({data:{text}} = await recognize('eng+spa')); }
      catch(_){ ({data:{text}} = await recognize('eng')); }

      const clean = (text||"").replace(/[\r\n]+/g," ").replace(/\s+/g," ").trim();
      if(clean) $("quoteText").value = clean;
      $("cropperOverlay").classList.remove("open");
      $("ocrStatus").textContent = clean ? "Text inserted into quote box." : "No text found.";
    }catch(e){
      console.error(e);
...
// If you provided a Google OAuth Client ID in config.js, store it in state
...
ensureDefaultBook(); bind(); updateLanguageUI();
