...
async function flushWebLLMPendingBooks(){
  ...
  for(const id of ids){
    ...
    if(book.quest && (book.quest.method === "webgpu-llm" || book.quest.method === "cloud-ai")) continue;
...
async function flushTransformersPendingBooks(){
  ...
  for(const id of ids){
    ...
    const synopsis = (book.synopsis || "").trim();
    if(!synopsis) continue;
    if(book.quest && (book.quest.method === "webgpu-llm" || book.quest.method === "window-ai" || book.quest.method === "transformers-js" || book.quest.method === "cloud-ai")) continue;
    await generateQuestObjectsForBook(book, { force: true, allowFallback: false, queueForAI: false });
  }
}
...
async function requestCloudQuestObjects(book, count, lang, sourceText, sameLanguage){
  try{
    ...
    const data = await res.json();
    return Array.isArray(data && data.objects) ? data.objects : null;
  }catch(_){
    return null;
  }
}
...
async function generateQuestObjectsForBook(book, options){
  ...
  try{
    ...
    if(wantsAI && cloudAiEnabled() && online){
      ...
      let objects = null;
      const raw = await requestCloudQuestObjects(book, count, lang, synopsis, sameLang);
